...
def part_1(data: str) -> Any:
    ...
    lines = data.split('\n')
    grid_x = grid_y = 70
    dest = Vector(grid_y, grid_x)
    valid_spots = {
        Vector(y, x) for y, x in itertools.product(*[range(grid_x + 1), range(grid_y + 1)])
    }
    corrupted = set()
    for line in lines[:1024]:
        x, y = map(int, re.findall("\d+", line))
        corrupted.add(Vector(y, x))
    valid_spots_check = valid_spots - corrupted
    tot = find_path(
        dest,
        valid_spots - corrupted
    )
...
def part_2(data: str) -> Any:
    ...
    for line in lines[:1024]:
        x, y = map(int, re.findall("\d+", line))
        valid_spots.remove(Vector(y, x))
    for line in lines[1024:]:
        x, y = map(int, re.findall("\d+", line))
        valid_spots.remove(Vector(y, x))
        if find_path(dest, valid_spots) == -1:
            return line
