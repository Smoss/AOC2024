...
def part_1(data: str) -> Any:
    ...
    for line in data.split("\n"):
        number = int(line)
        for _ in range(2000):
            number = ((number * 64) ^ number) % modulo_val
            number = ((number // 32) ^ number) % modulo_val
            number = ((number * 2048) ^ number) % modulo_val
        tot += number
...
def part_2(data: str) -> Any:
    tot = 0
    changes: dict[tuple[int, int, int, int], int] = defaultdict(int)
    for line in data.split("\n"):
        number = int(line)
        prev_number = number
        curr_series = []
        ...
        for _ in range(2000):
            ...
            number = ((number // 32) ^ number) % modulo_val
            number = ((number * 2048) ^ number) % modulo_val
            prev_sale = prev_number % 10
            curr_sale = number % 10
            curr_series.append((curr_sale - prev_sale))
            curr_series = curr_series[-4:]
            sale_price = curr_sale-prev_sale
            assert abs(curr_sale - prev_sale) < 10
            if len(curr_series) == 4 and tuple(curr_series) not in curr_changes:
                curr_changes[tuple(curr_series)] = curr_sale
            # print(curr_series)
        # print(curr_changes.get((-2, 1, -1, 3)))
        for key, val in curr_changes.items():
            changes[key] += val
        tot += number

        # print(line, number)
    tot = max(changes.values())
    return tot
