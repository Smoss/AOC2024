...
def part_1(data: str) -> Any:
    tot = 0
    pc_map = defaultdict(set)
    seen = set()
    for line in data.split("\n"):
        left, right = line.split("-")
        pc_map[left].add(right)
        pc_map[right].add(left)
        seen.add(left)
        seen.add(right)
    seen_list = sorted(list(seen))
    tri_sets = set()
    for i, pc_1 in enumerate(seen_list):
        pc_1_set = pc_map[pc_1]
        for pc_2 in seen_list[i+1:]:
            if pc_2 in pc_1_set:
                to_add = pc_1_set.intersection(pc_map[pc_2])
                for adder in to_add:
                    tri_sets.add(frozenset({pc_1, pc_2, adder}))
    # print(len(tri_sets))
    for tri_set in tri_sets:
        # print(tri_set)
        for pc in tri_set:
            if pc[0] == 't':
                tot+=1
...
def find_largest_set(pc_set: set[str], pc_map: dict[str, set[str]], result: tuple[str, ...]) -> tuple[str, ...]:
    if pc_set:
        next_val = next(iter(pc_set))
        next_val_set = pc_map[next_val]
        items_to_remove = set()
        for pc in pc_set:
            if pc != next_val and pc not in next_val_set:
                items_to_remove.add(pc)
        new_result = find_largest_set(pc_set - {next_val} - items_to_remove, pc_map, result + (next_val,))
        if items_to_remove:
            other_result = find_largest_set(pc_set - {next_val} - items_to_remove, pc_map, result)
            if len(other_result) > len(new_result):
                new_result = other_result
        return new_result
    else:
        return result
...
def part_2(data: str) -> Any:
    ...
    checked: set[str] = set()
    for pc in seen_list:
        set_to_check = (pc_map[pc] - checked)
        largest_containing_pc = find_largest_set(set_to_check, pc_map, (pc,))
        if len(largest_containing_pc) > len(largest_set):
            largest_set = largest_containing_pc
        checked.add(pc)
